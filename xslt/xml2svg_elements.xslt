...
<xsl:stylesheet version="1.0" 
  ...
  <xsl:template match="element">
    ...
    <svg xmlns="http://www.w3.org/2000/svg"
      ...
      <xsl:variable name="size_of_rect" select="10 * string-length(@name)"/>
      <xsl:choose>
  ...
  <xsl:template name="children">
    ...
    <xsl:if test="$max_height > 0">
      <xsl:call-template name="draw_element">
        ...
        <xsl:with-param name="size" select="10 * string-length(child[$index]/@id)"/>
      </xsl:call-template>     
      <xsl:call-template name="children">
  ...
  <xsl:template name="draw_element">
    ...
    <xsl:variable name="text_length" select="$size+10"/>
    <rect
       style="fill:green"
       id="{$name}"
       class="{$class}"
